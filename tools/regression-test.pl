...
# regression-test.pl: tests different versions of xavs
...
# - changing xavs option sets and or adding/removing option sets
...
{
    print "Regression test for xavs\n";
    print
"Usage:\n  perl tools/regression-test.pl --version=623 --version=624 --input=football_720x480p.yuv --input=foreman_352x288p.yuv --options='--me=dia --subme=2 --no-cabac'\n";
    print
      "Any number of versions, option sets, and input files may be given.\n";
    print
"Versions may be any svn revision, a comma-separated list of revisions, or 'current' for the version in the current directory.\n";
    exit;
}
...
foreach my $version (@versions) {
    ...
    if ( -e "test/xavs-$version/xavs" && ( $version ne "current" ) ) {
    ...
    if ( $version eq "current" ) {
        ...
        mkpath("test/xavs-$version");
        if ( !-e "xavs" ) { print("build failed \n"); exit 1; }
        copy( "xavs", "test/xavs-$version/xavs" );
        chmod( 0755, "test/xavs-$version/xavs" );
        next;
    }
    system(
"svn checkout -$version svn co https://xavs.svn.sourceforge.net/svnroot/xavs/trunk/ test/xavs-$version >/dev/null"
    );
    chdir("test/xavs-$version");
    system("(./configure && make) &> build.log");
    chdir("../..");
    if ( !-e "test/xavs-$version/xavs" ) { print("build failed \n"); exit 1; }
}
    ...
    foreach my $j ( 0 .. scalar(@input_files) - 1 ) {
        ...
        foreach my $k ( 0 .. scalar(@versions) - 1 ) {
            ...
            if ( -e "$outfile-$version.log" && ( $version ne "current" ) ) {
                ...
                system(
"test/xavs-$version/xavs --verbose $opt $file -o $outfile-$version.avs > $outfile-$version.log 2>&1"
                );
            }
...
# TODO dump (and check) frames
            ...
            if ( $k > 0 ) {
                ...
                $is_diff = 0;
                $is_diff ||= compare_logs( "$outfile-$version.log",
                    "$outfile-$baseversion.log" );
                $is_diff ||= compare_rawavs( "$outfile-$version.avs",
                    "$outfile-$baseversion.avs" );
...
sub parse_log_frame_stats {
    ...
    my @frames = ();
    while ( $logtext =~ m!xavs \[debug]: (frame=.*)!g ) {
        my $line = $1;
        if ( $line !~
m!frame=\s*(\d+)\s* QP=\s*(\d+)\s* NAL=(\d+)\s* Slice:(\w)\s* Poc:(\d+)\s* I:(\d+)\s* P:(\d+)\s* SKIP:(\d+)\s* size=(\d+) bytes PSNR Y:(\d+\.\d+) U:(\d+\.\d+) V:(\d+\.\d+)!
          )
        {
...
sub parse_log_overall_stats {
    ...
    if ( $logtext !~
m!xavs \[info\]: PSNR Mean Y:(\d+\.\d+) U:(\d+\.\d+) V:(\d+\.\d+) Avg:(\d+\.\d+) Global:(\d+\.\d+) kb/s:(\d+\.\d+)!
      )
    {
